# 1. TrendSurf Optima Nedir?

TrendSurf Optima, Türkiye'deki beş temel varlık sınıfını — BIST
hisseleri, TEFAS yatırım fonları, döviz kurları, kıymetli madenler ve
kriptoparalar — tek bir ekranda birlikte değerlendiren, kişisel bir
finansal takip ve sinyal terminalidir.

Sistem üç temel işi yapar: (1) piyasadaki binlerce varlığı sürekli
güncellenen, yaklaşık 15 dakika gecikmeli canlı verilerle tarayıp bir
bileşik puanla (Optima Skoru) sıralar, (2) sizin gerçek portföyünüzü
takip eder, kâr/zarar hesaplar ve size özel raporlar gönderir, (3)
elinizdeki bir varlık kâr realizasyonu fırsatı sunduğunda otomatik
olarak sizi uyarır.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#fff4e5;border-left:5px solid #e08a00;padding:14px 16px;border-radius:4px;"><p><strong>⚠ Yatırım Tavsiyesi Değildir</strong></p>
<p>TrendSurf Optima'daki hiçbir skor, sinyal, öneri veya hesaplama
yatırım tavsiyesi niteliği taşımaz. Sistem, geçmiş ve güncel piyasa
verilerini matematiksel olarak özetler. Tüm alım/satım kararları tamamen
size aittir.</p></td>
</tr>
</tbody>
</table>

**Bu Kılavuzu Nasıl Kullanmalısınız?**

Kılavuz, uygulamadaki sayfa sırasını takip eder. Her bölümde önce "ne
işe yarar" sonra "nasıl okunur/hesaplanır" anlatılır. Kılavuzun
sonundaki Terimler Sözlüğü ve Sık Sorulan Sorular bölümlerine, herhangi
bir noktada aklınıza takılan bir terim veya sorun için doğrudan
atlayabilirsiniz.

# 2. Başlangıç — Kayıt ve Giriş

**Hesap Oluşturma**

1.  Giriş ekranında "Kayıt Ol" sekmesine tıklayın.

2.  Ad Soyad, e-posta ve şifrenizi girin (en az 8 karakter).

3.  Hesabınız, sistem yöneticisinin onayından sonra aktif olur — onay
    bekleme süresi genellikle birkaç saat içindedir.

**Giriş Yapma**

- E-posta ve şifrenizle giriş yapın.

- "Beni Hatırla" kutusunu işaretlerseniz, oturumunuz 90 gün boyunca
  hatırlanır — her seferinde şifre girmeniz gerekmez.

- Tarayıcı şifre yöneticisi (Chrome, Edge, Firefox) bilgilerinizi
  kaydetmenize izin verebilir.

**Şifremi Unuttum**

- Giriş ekranında "Şifremi Unuttum" sekmesine tıklayın.

- E-posta adresinizi girin; sıfırlama bağlantısı e-postanıza gönderilir.

- Bağlantı 24 saat geçerlidir.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#eaf2fb;border-left:5px solid #1b6ef3;padding:14px 16px;border-radius:4px;"><p><strong>ⓘ Neden Onay Bekliyorum?</strong></p>
<p>TrendSurf Optima davetle büyüyen, küçük ölçekli bir platformdur. Her
yeni hesap, sistem yöneticisi tarafından manuel onaylanır. Bu, hem
güvenlik hem de sistem kaynaklarının kontrollü kullanımı
içindir.</p></td>
</tr>
</tbody>
</table>

# 3. Ana Sayfa — Portföy Optimizasyonu

Ana Sayfa, girdiğiniz bütçeyi ve risk tercihinizi temel alarak, o an
piyasadaki en güçlü sinyalli varlıklara genel bir optima skoru vererek,
aralarında kâr maksimizasyonunu gözetecek şekilde akıllıca dağıtan bir
öneri motorudur. Beş varlık sınıfını (BIST, TEFAS, Döviz, Maden, Kripto)
aynı anda değerlendirir.

## 3.1 Sol Panelden Ayarlar

| **Ayar**             | **Açıklama**                                                                                                         |
|----------------------|----------------------------------------------------------------------------------------------------------------------|
| Portföy Bütçesi (TL) | Yatırım düşündüğünüz toplam tutar. Örnek: 100.000.                                                                   |
| Risk Toleransı       | Düşük → Yüksek. Düşük risk daha çok TEFAS/döviz, yüksek risk daha çok hisse/kripto ağırlığı verir.                   |
| Max Varlık Sayısı    | Portföyde en fazla kaç farklı varlık önerilsin (kategori başına). Az sayı yoğunlaşma, çok sayı çeşitlendirme sağlar. |

## 3.2 Öneri Tablosu Sütunları

| **Sütun**        | **Açıklama**                                                                            |
|------------------|-----------------------------------------------------------------------------------------|
| Kategori         | BIST, TEFAS, Döviz, Maden, Kripto                                                       |
| Ticker           | Varlığın işlem sembolü                                                                  |
| Optima Skoru     | 0-100 arası bileşik puan (yüksek = daha güçlü pozisyon)                                 |
| Sinyal           | Güçlü Al / Kademeli Al / Tut-İzle / Sat / Net Sat                                       |
| RSI              | Göreceli Güç Endeksi (30 altı aşırı satım, 70 üzeri aşırı alım)                         |
| 1A Getiri %      | Son 1 aylık fiyat getirisi                                                              |
| Emir Fiyatı      | Güncel piyasa fiyatı — limit emir girerken referans alın                                |
| Birim            | Bütçenize göre önerilen alım adedi/lot                                                  |
| Gerçek Tutar (₺) | Birim × Emir Fiyatı — birim tam sayıya yuvarlandığı için Hedef Tutar'dan küçük olabilir |
| Hedef Tutar (₺)  | O varlığa ayrılan bütçe payı                                                            |

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#eaf2fb;border-left:5px solid #1b6ef3;padding:14px 16px;border-radius:4px;"><p><strong>ⓘ Bir Satıra Tıklayın</strong></p>
<p>Tabloda bir satırı seçtiğinizde (satırın herhangi bir hücresine
tıklayarak), sayfanın altında o varlığın fiyat grafiği, teknik
göstergeleri ve detaylı analizi açılır.</p></td>
</tr>
</tbody>
</table>

## 3.3 Bütçe Nasıl Dağıtılıyor? (Su Doldurma Mantığı)

Sistem önce risk toleransınıza göre bütçenizi kategoriler arasında
(BIST/TEFAS/Döviz/Maden/Kripto) paylaştırır, sonra her kategori içinde o
kategorinin bütçesini en güçlü sinyalli varlıklar arasında eşit olarak
bölüştürür.

Bazı varlıkların birim fiyatı (özellikle gram altın gibi pahalı
varlıklar), payına düşen bütçeden daha yüksek olabilir. Böyle bir
durumda o varlık listeden çıkarılır ve payı, aynı kategorideki diğer
varlıklara yeniden dağıtılır — bu işlem, kategorideki tüm varlıklar ya
karşılanana ya da kategori tamamen boşalana kadar tekrarlanır.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#eafaf1;border-left:5px solid #1fa35a;padding:14px 16px;border-radius:4px;"><p><strong>✓ Neden Önemli?</strong></p>
<p>Bu sayede önerilen listede asla "0 birim, 0 TL" gibi anlamsız bir
satır görmezsiniz — listede yer alan her varlık, gerçekten en az 1 birim
alınabilir demektir. Eğer bir kategoride hiçbir varlık bütçenizi
karşılamıyorsa, sistem bunu açıkça bir bilgi notuyla belirtir.</p></td>
</tr>
</tbody>
</table>

## 3.4 Optima Skoru Nasıl Hesaplanır?

Sistem dört faktörü ağırlıklı olarak birleştirir:

- RSI Zonu — %25 ağırlık. Aşırı satım bölgesi (RSI \< 30) yüksek puan
  alır.

- Momentum / Getiri — %35 ağırlık. Son aylık ve haftalık getiri
  trendleri.

- Volatilite — %15 ağırlık. Aşırı oynaklık puanı düşürür.

- Temel Analiz — %25 ağırlık. P/E, beta, temettü verimi (BIST için KAP
  verisi).

## 3.5 Kategori Dağılımı (Pasta Grafiği)

Tablonun altındaki 3 boyutlu pasta grafiği, önerilen portföyünüzün
kategoriler arasındaki dağılımını görsel olarak özetler. Her dilimin
büyüklüğü, o kategoriye ayrılan bütçe payını gösterir.

## 3.6 Detaylı Analiz Bölümü

Tabloda bir satır seçtiğinizde, sayfanın altında o varlığa özel bir
detaylı analiz bölümü açılır: mum grafiği (candlestick chart), işlem
hacmi göstergesi, teknik göstergeler (MA20/MA50, 52 Haftalık
Yüksek/Düşük, MACD) ve Optima Skoru'nun bileşenlerine ayrıldığı bir skor
dökümü yer alır.

**Mum Grafiği (Candlestick) Nasıl Okunur?**

Her "mum", belirli bir zaman dilimindeki (genellikle bir gün) fiyat
hareketini özetler. Mumun gövdesi (kalın kısmı), o gün açılış ve kapanış
fiyatları arasındaki farkı gösterir; gövdeden yukarı ve aşağı uzanan
ince çizgiler (fitil/iğne) ise gün içinde görülen en yüksek ve en düşük
fiyatı gösterir.

- Yeşil (veya dolu) mum: kapanış fiyatı açılıştan yüksek — o gün
  alıcılar baskın olmuş.

- Kırmızı (veya boş) mum: kapanış fiyatı açılıştan düşük — o gün
  satıcılar baskın olmuş.

- Uzun fitiller: gün içinde fiyatın gövde dışına çıkıp geri döndüğünü,
  yani kararsızlık/mücadele olduğunu gösterir.

- Art arda aynı yönde büyük gövdeli mumlar: güçlü, kararlı bir trend
  olduğuna işaret eder.

**İşlem Hacmi Nasıl Okunur?**

Mum grafiğinin altındaki çubuklar, o günkü işlem hacmini (alınıp satılan
toplam miktarı) gösterir. Hacim, bir fiyat hareketinin "ne kadar ciddiye
alınması gerektiğini" anlamak için kullanılır:

- Yüksek hacimle birlikte gelen bir fiyat artışı/düşüşü: hareketin
  arkasında güçlü bir katılım var demektir — daha güvenilir bir sinyal
  sayılır.

- Düşük hacimle gelen bir fiyat hareketi: az sayıda işlemle oluşmuş
  olabilir — kalıcılığı daha şüphelidir, ani geri dönüşlere daha
  açıktır.

- Fiyat yükselirken hacmin azalması: yükselişin gücünü kaybediyor
  olabileceğine dair erken bir uyarı sayılabilir.

**Skor Bileşimi**

Bu bölümde, o varlığın Optima Skoru'nun hangi alt bileşenlerden (RSI
zonu, momentum, volatilite, temel analiz) ne kadar puan aldığı ayrı ayrı
gösterilir — böylece skorun "neden" bu seviyede olduğunu
anlayabilirsiniz.

# 4. Portföyüm — Pozisyon Takibi

Bu sayfada gerçek alımlarınızı kayıt edip takip edersiniz. Sistem güncel
fiyatları otomatik günceller, kâr/zarar hesaplar ve isterseniz kâr
maksimizasyonu için ayarlayacağınız kriterlere göre size e-posta raporu
gönderir. Sistem portföyünüzün muhasebesini yapmaz, sadece anlık
durumunu gösterir.

## 4.1 Yeni Pozisyon Ekleme

1.  "Yeni Pozisyon Ekle" bölümünü açın.

2.  Kategori seçin (BIST, TEFAS, Döviz, Maden, Kripto).

3.  Listeden varlık seçin (örn: GARAN, USD/TRY, ATOM).

4.  Birim Türü otomatik seçilir: BIST hisseleri → Lot, gram altın/gümüş
    → Gram, diğerleri (kripto, döviz, fon) → Adet.

5.  Adet/Miktar, Alış Fiyatı (TL) ve Alış Tarihi girin.

6.  İsterseniz Not ekleyin (hangi bankadan, kampanya bilgisi vb.).

7.  "Pozisyon Ekle" butonuna tıklayın.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#eaf2fb;border-left:5px solid #1b6ef3;padding:14px 16px;border-radius:4px;"><p><strong>ⓘ Türk Sayı Formatı</strong></p>
<p>Sistem boyunca Türk biçimi kullanılır: nokta = binlik ayracı, virgül
= ondalık. Örnek: 1.234,5670 (bin iki yüz otuz dört virgül beş altı yedi
sıfır).</p></td>
</tr>
</tbody>
</table>

## 4.2 Portföy Tablosu Sütunları

| **Sütun**    | **Açıklama**                          |
|--------------|---------------------------------------|
| Ticker       | Varlık sembolü                        |
| Kategori     | BIST / TEFAS / Döviz / Maden / Kripto |
| Adet         | Sahip olduğunuz miktar                |
| Birim        | Lot / Gram / Adet                     |
| Alış Fiyatı  | Birim başına alış maliyetiniz         |
| Güncel Fiyat | Canlı veriden anlık fiyat             |
| Toplam Değer | Adet × Güncel Fiyat                   |
| K/Z (TL)     | Toplam Değer − (Adet × Alış)          |
| K/Z %        | (Güncel − Alış) / Alış × 100          |
| Optima Skor  | Varlığın mevcut sinyal puanı (0-100)  |
| Alış Tarihi  | Pozisyonu açtığınız gün               |

## 4.3 Pozisyon Düzenleme / Silme

- Tabloda sağdaki düzenle ikonu ile mevcut pozisyonu
  güncelleyebilirsiniz.

- Çoklu seçim ile birden fazla pozisyonu aynı anda silebilirsiniz.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#fff4e5;border-left:5px solid #e08a00;padding:14px 16px;border-radius:4px;"><p><strong>⚠ Dikkat</strong></p>
<p>Silinen pozisyonlar geri alınamaz.</p></td>
</tr>
</tbody>
</table>

# 5. E-Posta ve Uyarı Ayarları

## 5.1 Günlük Optimizasyon ve Portföy Raporu

Sistem her gün ayarlayacağınız saatlerde, 2 kere otomatik olarak
Optimizasyon ve Portföy raporu gönderir. Optimizasyon raporunda bir
bütçe değeri girilmediyse, kendiliğinden 20.000 TL yatırım bütçesinin
optimize edilerek dağıtıldığı tablo gönderilir. Optimizasyon tablosunun
altında, önceden kayıt ettiğiniz Portföy raporunuz yer alır.
Sidebar'daki "E-Posta Ayarları" panelinden gönderim saatlerini
ayarlarsınız.

- Birinci Gönderim Saati: Sabah raporu için (örn: 08:30).

- İkinci Gönderim Saati: Akşam raporu için (örn: 17:00). Boş
  bırakırsanız sadece sabah raporu gelir.

- "Kaydet" butonuna basınca değişiklikler kayıt edilir.

**Rapor İçeriği**

Mail içinde portföyünüzdeki her varlık için: Ticker, kategori, adet,
alış/güncel fiyat, toplam değer, K/Z (TL ve %), Optima Skor ve mevcut
sinyal, ayrıca genel bir özet blok (toplam değer, toplam K/Z, en iyi/en
kötü performans) yer alır.

## 5.2 Kâr Realizasyonu Uyarı Sistemi

Portföyünüzdeki her varlığın gördüğü en yüksek fiyatı ("peak") sürekli
izler; bu tepe noktasından belirlediğiniz orandan fazla düşüş
gerçekleştiğinde otomatik e-posta gönderir. Sistem, siz uygulamada
olmasanız bile 7/24 arka planda çalışır.

| **Ayar**              | **Ne İşe Yarar**                                                                |
|-----------------------|---------------------------------------------------------------------------------|
| Uyarı Sistemi Aktif   | İşaretli değilse hiçbir uyarı gelmez — geçici olarak durdurmak için kullanılır. |
| Düşüş Eşiği           | Peak fiyattan kaç % düşüşte uyarı gelsin (önerilen: %3).                        |
| Sadece Kârdayken Uyar | AÇIK ise, yalnızca güncel fiyat alış fiyatınızdan yüksekse uyarı gelir.         |
| Kontrol Sıklığı       | 15 / 30 / 60 dakika — sistemin sizin için ne sıklıkla kontrol yapacağı.         |
| Uyarı Tekrar Mantığı  | Peak Kırıldıkça (önerilen) / Bir Kez Gönder / Saatte Bir Tekrarla.              |
| Tavsiye Emir Fiyatı   | Peak × (1−eşik) / Şu anki fiyat / Sadece bilgi (fiyat önerisi yok).             |

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#eafaf1;border-left:5px solid #1fa35a;padding:14px 16px;border-radius:4px;"><p><strong>✓ Önerilen Başlangıç Ayarları</strong></p>
<p>Uyarı sistemi: Aktif · Düşüş eşiği: %3 · Sadece kârdayken uyar: Açık
· Kontrol sıklığı: 30 dk · Uyarı modu: Peak kırıldıkça. Bu ayarlarla
sistem ortalama 1-2 günde bir uyarı verir.</p></td>
</tr>
</tbody>
</table>

**Manuel Test ve Peak Sıfırlama**

- "Şimdi Kontrol Et (Test)" — mail göndermeden sistemin ne göreceğini
  gösterir.

- "Bekleyen Uyarıları Mail Gönder" — test sonrası gerçekten mail
  göndermek için.

- "Tüm Peak'leri Sıfırla" — portföyünüzde büyük değişiklik yaptığınızda
  kullanışlıdır; geri alınamaz.

**Hafta Sonu Davranışı**

Cumartesi/Pazar günleri BIST, TEFAS, Döviz ve Maden piyasaları kapalı
olduğundan, sistem hafta sonu sadece kripto varlıklar için kontrol
yapar.

# 6. BIST — Türk Hisse Senetleri

Yaklaşık 770 Borsa İstanbul hissesini Optima Skoru'na göre sıralı
listeler.

## 6.1 Hisse Detayı

- Her hissenin linki Detaylı Analiz sayfasına açılır.

- Teknik Göstergeler; Optima Skor, RSI, 1A Getiri, P/E, beta gibi
  detaylar tek bakışta görünür.

- Mum grafik, Temel Analiz Raporu ve Skor Bileşimi'ni burada
  görebilirsiniz.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#eaf2fb;border-left:5px solid #1b6ef3;padding:14px 16px;border-radius:4px;"><p><strong>ⓘ Veri Kaynağı</strong></p>
<p>BIST fiyatları öncelikli olarak canlı kaynaktan çekilir; erişilemezse
otomatik olarak yedek kaynağa geçilir. Gün içi fiyat değişimleri birkaç
dakika gecikmeyle sisteme yansır.</p></td>
</tr>
</tbody>
</table>

# 7. TEFAS — Yatırım Fonları

Binlerce yatırım fonunu Türkiye Elektronik Fon Alım Satım Platformu
(TEFAS) verileriyle listeler.

## 7.1 Fon Türü Filtresi

- Hisse Senedi Yoğun Fonlar

- Karma ve Değişken Fonlar

- Tahvil Fonları

- Altın ve Kıymetli Maden Fonları

- Para Piyasası ve Kısa Vadeli Fonlar

- Serbest Fonlar

## 7.2 Getiri Karşılaştırması

Her fon için 1 Aylık, 3 Aylık, 6 Aylık ve 1 Yıllık getiriler yan yana
görüntülenir — performans karşılaştırması tek bakışta yapılabilir.

## 7.3 TEFAS Detayı

Her fonun detay sayfasında Teknik Analiz raporları, Teknik Göstergeler,
Mum Grafiği, Getiri ve risk analizleri ile fon karnesi, portföy dağılımı
ve son işlem fiyatı görülebilir.

# 8. Döviz, Madenler ve Kriptolar

## 8.1 Döviz

- 12 TRY bazlı kur izlenir: USD, EUR, GBP, CHF, JPY ve diğerleri.

## 8.2 Madenler

- 10 maden: Altın (Gram), Gümüş (Gram), Platin, Paladyum, Bakır vb.

- Gram altın ve gram gümüş için doğrudan TL/gram Türkiye fiyatı
  kullanılır.

## 8.3 Kriptolar

- 18 kripto para: BTC, ETH, SOL, ADA, ATOM, AVAX, DOT, MATIC, TON vb.

- BTC/TRY ve ETH/TRY için doğrudan TL fiyat kullanılır.

- Kripto piyasası 7/24 aktif — kâr realizasyonu uyarı sistemi hafta sonu
  sadece bu kategoride çalışır.

- Kriptoların da BIST'de olduğu gibi işlem hacimleri grafikte
  gösterilmektedir.

# 9. Halka Arz — Yaklaşan ve Devam Eden IPO'lar

Bu sayfa, Borsa İstanbul'da halka arz süreci devam eden veya yakında
başlayacak şirketleri, KAP (Kamuyu Aydınlatma Platformu) üzerinden
otomatik olarak takip eder. Şirketin resmi Fiyat Tespit Raporu
yayınlandığında, sistem bu raporu otomatik indirip analiz eder.

## 9.1 Tablo Sütunları

| **Sütun**               | **Açıklama**                                                                                  |
|-------------------------|-----------------------------------------------------------------------------------------------|
| Tarih                   | KAP bildirim tarihi                                                                           |
| Kod                     | Şirketin işlem göreceği/görecek sembol                                                        |
| Şirket                  | Şirket unvanı                                                                                 |
| Durum                   | Sürecin aşaması (örn. "Onaylandı, Talep Toplanıyor", "Yakında")                               |
| Arz Fiyatı (TL)         | Fiyat Tespit Raporu'nda belirlenen halka arz fiyatı                                           |
| İskonto (%)             | Arz fiyatının, bağımsız değerleme yöntemlerine göre hesaplanan "gerçek değer"e göre iskontosu |
| Graham Değeri (TL)      | Benjamin Graham'ın klasik formülüyle hesaplanan bağımsız değer tahmini                        |
| Çarpan Bazlı Değer (TL) | Sektör çarpanlarına göre hesaplanan bağımsız değer tahmini                                    |
| Rapor                   | Fiyat Tespit Raporu'nun KAP'taki orijinaline link                                             |

## 9.2 Arz Fiyatı Renklendirmesi Nasıl Okunur?

"Arz Fiyatı" hücresi, Graham Değeri ve Çarpan Bazlı Değer ile
karşılaştırılarak otomatik renklendirilir:

- Yeşil — Arz fiyatı, bağımsız değerleme yöntemlerinin altında;
  potansiyel olarak cazip bir iskontoya işaret eder.

- Sarı — Arz fiyatı, bağımsız değerlemeye yakın; belirgin bir iskonto
  veya prim yok.

- Kırmızı — Arz fiyatı, bağımsız değerlemenin üzerinde; prim ödeniyor
  olabilir.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#fff4e5;border-left:5px solid #e08a00;padding:14px 16px;border-radius:4px;"><p><strong>⚠ Bu Bir Yatırım Tavsiyesi Değildir</strong></p>
<p>Graham Değeri ve Çarpan Bazlı Değer, sadece raporda açıklanan
finansal verilerden matematiksel olarak türetilen tahminlerdir. Sayfada
hiçbir şirkete özel getiri beklentisi belirtilmez — yalnızca geçmiş
halka arzların ilk gün ortalama istatistikleri gösterilir.</p></td>
</tr>
</tbody>
</table>

## 9.3 Bazı Şirketlerde Graham/Çarpan Değeri Neden Boş?

Fiyat Tespit Raporları KAP'ta genellikle taranmış (OCR gerektiren) PDF
formatında yayınlanır. Bazı raporların görüntü kalitesi düşük olduğunda,
sistem yanlış bir rakam üretip sizi yanıltmaktansa, o alanı bilinçli
olarak boş bırakır. Bu, bir hata değil, güvenlik önlemidir.

## 9.4 "Zorla Yenile" Butonu

Sistem, Fiyat Tespit Raporlarını her gece otomatik olarak arka planda
işler. Yeni bir rapor yayınlandığında veya bir önceki işlemede hata
oluştuğunda, bu butonla manuel olarak yeniden işleme tetiklenebilir.
Buton, daha önce başarıyla işlenmiş kayıtları atlayıp sadece
eksik/hatalı olanları tekrar dener.

# 10. Temettü Takip

BIST Temettü Endeksi (XTMTU) üyesi — düzenli nakit temettü dağıtan —
şirketleri, temettü getirisi ve fiyat momentumunu bir arada göstererek
karşılaştırmanızı sağlar.

## 10.1 Tablo Sütunları

| **Sütun**               | **Açıklama**                                          |
|-------------------------|-------------------------------------------------------|
| Ticker / Şirket         | Varlık sembolü ve unvanı                              |
| Fiyat (₺)               | Son kapanış fiyatı                                    |
| Temettü/Hisse (₺)       | Hisse başına ödenen/duyurulan brüt temettü tutarı     |
| Temettü Verimi %        | Temettü/Hisse ÷ Fiyat × 100                           |
| Ex-Date                 | Haklardan düşme tarihi (bkz. 10.2)                    |
| Sıklık                  | Temettü ödeme sıklığı (Yıllık, Yarı Yıllık vb.)       |
| 1A Getiri %             | Son 1 aylık fiyat getirisi                            |
| Optima Skor             | TrendSurf Optima bileşik puanı                        |
| Tahmini Toplam Getiri % | 1 Aylık Momentum + Temettü Verimi (kaba bir gösterge) |
| KAP                     | Şirketin KAP sayfasına link                           |

## 10.2 Ex-Date Nedir, Neden Önemli?

Ex-Date (haklardan düşme tarihi), temettüyü almak için hisseye en geç ne
zamana kadar sahip olmanız gerektiğini belirtir. Temettüyü almak için
hisseye bu tarihten önceki gün kapanışında sahip olunması gerekir;
ex-date'te veya sonrasında yapılan alım, o dönemin temettüsünü kaçırır.

<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
<tbody>
<tr class="odd">
<td style="background-color:#eafaf1;border-left:5px solid #1fa35a;padding:14px 16px;border-radius:4px;"><p><strong>✓ Turuncu Vurgu</strong></p>
<p>Ex-Date'e 14 gün veya daha az kalan satırlar tabloda turuncu renkle
vurgulanır — bu, "harekete geçmeniz gereken" bir zaman penceresine
işaret eder.</p></td>
</tr>
</tbody>
</table>

## 10.3 Bu Tabloyu Nasıl Kullanabilirsiniz? — Üç Yöntem

- **1. Temettü geliri hesabı:** Portföy Bütçesi × Temettü Verimi % = o
  hisseye yatırılırsa yıllık beklenen brüt nakit temettü tutarı (stopaj
  bu hesaba dahil değildir, ayrıca düşülmelidir).

- **2. Ex-date takvimi:** Yaklaşan (turuncu vurgulu) ve yüksek verimli
  satırları filtreleyerek, o ayki temettü hakkını kaçırmadan almak
  istediğiniz hisselerin bir listesini çıkarabilirsiniz.

- **3. Kalite filtresi:** Sadece en yüksek Temettü Verimi %'ne bakmak
  yanıltıcı olabilir — bazen düşen fiyat verimi yapay olarak şişirir.
  Optima Skor ve 1A Getiri sütunlarıyla birlikte değerlendirin.

# 11. Makro Göstergeler

MKK Veri Analiz Platformu (vap.org.tr) bağlantıları ve özet veriler.

- Genel Bakış

- Yaş Grupları Analizi

- Yerli / Yabancı Yatırımcı Dağılımı

- BIST Endeks Bazlı Portföy

- Finansal Oranlar

- REKS Risk İştahı Endeksi

- MKK Aylık Bülteni

Her karta tıklayınca ilgili VAP sayfası yeni sekmede açılır.

# 12. Terimler Sözlüğü — Değerler Ne Anlama Gelir?

Bu bölüm, uygulama genelinde karşınıza çıkan sayısal göstergelerin ne
ölçtüğünü ve nasıl okunması gerektiğini açıklar.

| **Terim**                                                  | **Anlamı**                                                                                                                                                                                                                                                         |
|------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Optima Skoru                                               | 0-100 arası bileşik puan. RSI zonu, momentum, volatilite ve temel analizin ağırlıklı ortalamasıdır. Yüksek puan Güçlü Al, düşük puan Net Sat yönünde bir sinyale karşılık gelir (bkz. Sinyal).                                                                     |
| RSI (Göreceli Güç Endeksi)                                 | 0-100 arası bir momentum göstergesi. 30'un altı "aşırı satım" (fiyat aşırı düşmüş olabilir), 70'in üzeri "aşırı alım" (fiyat aşırı yükselmiş olabilir) bölgesi sayılır. Tek başına alım/satım sinyali değildir, diğer göstergelerle birlikte değerlendirilmelidir. |
| MACD                                                       | İki farklı hareketli ortalamanın (genelde 12 ve 26 günlük) farkına dayanan bir trend/momentum göstergesi. MACD çizgisinin sinyal çizgisini yukarı kesmesi genelde yükseliş, aşağı kesmesi düşüş sinyali olarak yorumlanır.                                         |
| MA20 / MA50                                                | Son 20 veya 50 günün ortalama kapanış fiyatı (hareketli ortalama). Fiyat bu ortalamaların üzerindeyse kısa/orta vadeli trend yukarı, altındaysa aşağı yönlü kabul edilir.                                                                                          |
| 52 Haftalık Yüksek/Düşük                                   | Son bir yıl içinde görülen en yüksek ve en düşük fiyat. Güncel fiyatın bu aralıktaki konumu, varlığın "ucuz mu pahalı mı" olduğuna dair kaba bir fikir verir.                                                                                                      |
| 1A Getiri %                                                | Son 1 aylık fiyat değişim yüzdesi. Kısa vadeli momentumu gösterir.                                                                                                                                                                                                 |
| P/E (Fiyat/Kazanç Oranı)                                   | Hisse fiyatının, şirketin hisse başına kârına bölünmesiyle bulunur. Düşük P/E genellikle "ucuz", yüksek P/E "pahalı veya büyüme beklentisi yüksek" olarak yorumlanır — sektörler arası doğrudan kıyaslama yanıltıcı olabilir.                                      |
| Beta                                                       | Bir hissenin piyasa (endeks) hareketlerine göre oynaklığı. Beta \> 1 endeksten daha oynak, Beta \< 1 endeksten daha az oynak demektir.                                                                                                                             |
| Graham Değeri                                              | Benjamin Graham'ın klasik "içsel değer" formülüyle (hisse başına kâr ve büyüme oranı temelli) hesaplanan bağımsız bir değer tahmini. Halka Arz sayfasında Arz Fiyatı ile karşılaştırılır.                                                                          |
| Çarpan Bazlı Değer                                         | Sektördeki benzer şirketlerin ortalama çarpanlarına (F/K, PD/DD vb.) göre hesaplanan bağımsız bir değer tahmini.                                                                                                                                                   |
| İskonto %                                                  | Arz fiyatının, bağımsız değerleme tahminlerine göre ne kadar ucuz veya pahalı olduğunu gösteren yüzde fark.                                                                                                                                                        |
| Temettü Verimi %                                           | Hisse başına temettü tutarının, güncel fiyata bölünmesiyle bulunur. Sadece fiyat artışından değil, düzenli nakit dağıtımından elde edilen getiriyi ölçer.                                                                                                          |
| Ex-Date                                                    | Temettü hakkının düştüğü tarih. Bu tarihten önceki günün kapanışında hisseye sahip olunması, o dönemin temettüsünü almak için gereklidir.                                                                                                                          |
| K/Z (TL) ve K/Z %                                          | Portföyünüzdeki bir pozisyonun mutlak (TL) ve yüzdesel kâr/zararı. Güncel değer ile alış maliyeti arasındaki farktır.                                                                                                                                              |
| Peak (Tepe Fiyat)                                          | Kâr realizasyonu uyarı sisteminin izlediği, bir varlığın şimdiye kadar gördüğü en yüksek fiyat. Bu tepeden belirlediğiniz oranda düşüş olduğunda uyarı tetiklenir.                                                                                                 |
| Sinyal (Güçlü Al / Kademeli Al / Tut-İzle / Sat / Net Sat) | Optima Skoru'nun kategorik karşılığıdır; yüksekten düşüğe doğru sıralanır: Güçlü Al, Kademeli Al, Tut-İzle, Sat, Net Sat.                                                                                                                                          |

# 13. Sık Sorulan Sorular

**Veriler ne sıklıkla güncellenir?**

Fiyat verileri gün içinde düzenli aralıklarla otomatik güncellenir.
TEFAS/Halka Arz gibi daha az sık değişen veriler birkaç saatlik
önbellekten gelir.

**Portföy verilerim kayboldu, neden?**

Portföyünüz güvenli bir veritabanında saklanır, kaybolmaz. Çıkış yapıp
tekrar giriş yapmayı deneyin; sorun devam ederse sistem yöneticisiyle
iletişime geçin.

**Kâr realizasyonu uyarısı ne kadar gecikmeyle gelir?**

Sisteminizin kontrol sıklığı ayarına bağlı olarak, tetiklenmeden mail
gelene kadar birkaç dakika ile bir saat arası gecikme olabilir.

**Aynı varlık için defalarca mail alabilir miyim?**

Uyarı tekrar modunuza bağlıdır: "Peak Kırıldıkça" modunda aynı tepe için
bir kez, yeni bir tepe oluşursa tekrar gelir.

**Hafta sonu BIST hissem düşse mail gelir mi?**

Hayır. Hafta sonu piyasalar kapalı olduğu için sadece kripto varlıklar
için kontrol yapılır.

**Düşüş eşiğini çok düşük yaparsam ne olur?**

Mail kutunuz sık uyarılarla dolabilir. %2'nin altındaki değerler
önerilmez.

**Uygulama yavaş açılıyor?**

Uzun süre kullanılmayınca sistem uyku moduna geçebilir; ilk açılış 1-2
dakika sürebilir, sonrası hızlanır.

**Ana Sayfa'daki tablo mobilde neden farklı görünüyor?**

Tablo, telefon ekranlarında otomatik olarak tek sütunlu bir görünüme
geçer; tüm bilgiler yine de erişilebilir durumdadır.

**Halka Arz sayfasında bazı satırlarda Graham/Çarpan değeri neden yok?**

İlgili şirketin Fiyat Tespit Raporu düşük kaliteli/taranmış bir belge
olduğunda, sistem yanlış rakam üretmemek için bu alanı bilinçli olarak
boş bırakır.

**Veri kaynağı güvenilir mi?**

Her varlık sınıfı için birden fazla yedek kaynak kullanılır; birincil
kaynaktan veri alınamazsa sistem otomatik olarak yedek kaynağa geçer.
